...
class ProductApi{
    ...
    Future<Product> getStock(String data) async{
      final response = await _client.post("${Paths.BASEURL}${Paths.ENDPOINT_STOCK}",body:encryptAESCryptoJS(data,Paths.KEY),headers: _headers);
      if(response.statusCode == 200){
        Product product = Product.fromJson(json.decode(decryptAESCryptoJS(response.body, Paths.KEY)));
        return product;
      }else{
        return null;
      }
    ...
    Future<Product> getAllWarehouse() async{
      final response = await _client.get("${Paths.BASEURL}${Paths.ENDPOINT_STOCK}");
      if(response.statusCode == 200){
        Product product = Product.fromJson(json.decode(decryptAESCryptoJS(response.body, Paths.KEY)));
        return product;
      }else{
